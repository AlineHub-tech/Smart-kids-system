...
export default function Dashboard(){
  const { members, attendanceRecords, announcements } = useContext(AppContext);
  const todayStr = new Date().toISOString().slice(0,10);
  const todays = attendanceRecords.filter(r=> r.date === todayStr);
  const absentCount = todays.filter(r=> r.status === 'absent').length;
